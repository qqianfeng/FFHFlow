...
def show_dataloader_grasp(bps_path, obj, centr_T_mesh, palm_pose_mesh, palm_pose_centr, gazebo_obj_path,rendered_pcd=False):
    ...
    if rendered_pcd is False:
        rendered_pcd = utils.load_rendered_pcd(bps_path,convert_to_centroid=True)
...
def show_dataloader_grasp_with_pcd_in_world_frame(obj,
    ...
    Args:
        ...
        rendered_pcd (_type_): open3d point cloud data in world frame
...
def show_generated_grasp_distribution_pos_neg(pcd_path,
    ...
    for i in range(grasps[1]['rot_matrix'].shape[0]):
        ...
        if i == highlight_idx:
            ...
            frame = o3d.geometry.TriangleMesh.create_coordinate_frame(0.015).transform(
                palm_pose_centr)
        frames.append(frame)
