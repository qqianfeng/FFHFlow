...
from .backbones import PointNetfeat, FFHGenerator, BPSMLP, BPSVAE
from .heads import LocalInnFlow
...
class FFHFlowVAE(pl.LightningModule):
    ...
    def __init__(self, cfg: CfgNode):
        ...
        self.backbone = BPSVAE()

        # # free param in backbone
        # for param in self.backbone.parameters():
        #     param.requires_grad = False

        self.flow = LocalInnFlow(cfg)
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        mu, logvar = self.backbone.encode(batch)

        std = logvar.exp().pow(0.5)
        z_vae = torch.distributions.normal.Normal(mu, std)
        vae_samples = z_vae.rsample(num_samples)

        # basic distribution z
        mean = torch.log(torch.tensor([0.01, 0.5, 1, 0.01]))
        sigma = 0.5
        covariance = sigma ** 2 * torch.eye(4)
        gaussian = distributions.MultivariateNormal(
            loc=mean, covariance_matrix=covariance
        )
        z_samples = gaussian.sample(num_samples)

        # TODO: check dim
        z_all = torch.cat([vae_samples,z_samples],dim=1
                          )
        # If ActNorm layers are not initialized, initialize them
        if not self.initialized:
            self.initialize(batch)

        log_prob, pred_pose_6d = self.flow(z=z_all)
